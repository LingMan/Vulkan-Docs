...
include::{generated}/api/structs/VkSwapchainTimingInfoEXT.adoc[]
  ...
  * pname:presentAtNearestRefreshCycle specifies whether the presentation is
    allowed to target the refresh cycle that is nearest to the target present
    time, when one is provided. If ename:VK_FALSE, a targeted present stage
    must: not complete before the target present time specified via
